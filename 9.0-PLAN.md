...
- **Mikser-to-mikser composition** (v10 territory). The scaling answer for big corpora isn't "make one mikser hold N million entities"; it's "compose N miksers, each holding a domain-sized corpus, talking to each other through existing surfaces" (`mikser-io-api` for read-across, `mikser-io-mcp` for agent reach, `mikser-io-shares` for shared output). The one missing piece — a `mikser-io-source-mikser` consumer plugin that mounts another mikser's `/api` SSE feed as a virtual collection — and the cross-mikser refs story belong to v10. Until that lands, the substrate's design center stays "one folder, one mikser, domain-sized catalog." This also kills any 9.0-era pressure to add output streaming / iterator-first API redesigns / journal chunked commits — those exist to make a single process scale, which isn't the v10 direction.
