...
**Live MCP server for AI agents.** Start mikser with `--mcp` and any MCP-compatible client — Claude Desktop, Claude Code, ChatGPT, custom agents — connects to your running engine. The agent can list and read your catalog, write new content (markdown, layouts, configuration — whatever your collections accept), render any layout for preview without dirtying the output folder, and watch every engine log line as it streams in. Plugins extend the tool surface the same way they mount HTTP routes — install the plugin, the agent gets new verbs.

**What that looks like.** *"Draft three hero-section variants and show me previews"* — one chat turn, three writes, three inline HTML/PDF previews, no leftover artifacts. *"Why didn't the navigation update?"* — the agent reads the engine's rolling log buffer and sees exactly what your terminal saw. Operator, AI, and any observer dashboard share the same engine and the same logs, because mikser is single-tenant by design. See [`documentation/mcp.md`](./documentation/mcp.md) for the full tool surface and twelve worked scenarios.
