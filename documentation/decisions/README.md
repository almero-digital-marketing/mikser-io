...
| [0008](./0008-mcp-ui-action-delivery.md) | MCP-UI action delivery: `mikser_preview_ui` returns HTML synchronously; clicks ride the [MCP Apps spec](https://github.com/modelcontextprotocol/ext-apps) JSON-RPC over postMessage channel as `tools/call` against a separate app-callable `mikser_ui_action` tool (`_meta.ui.visibility = ['app']`). Action allow-list is the auth boundary. Optional `mcpUi.handler.url` forwards the action to an external webhook (HMAC-signed if `handler.secret` set); handler failures fall back to pure relay. No HTTP delivery surface, no suspended-tool model. | Accepted |
