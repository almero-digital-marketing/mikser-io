...
| [0009](./0009-database-engine-substrate.md) | Sqlite is the engine's persistence substrate. Single `runtime/mikser.sqlite` file holds catalog/refs/manifest as `mikser_entities` / `mikser_refs` / `mikser_snapshots` tables. Plugins register schemas via `registerSchema(name, sql)` + `useDatabase()`. Sift→SQL translator with indexed pushdown, LRU cache for `findById`, worker-side read-only sqlite for sync template helpers. Replaces `Map<id, entity>` + NDJSON across all three subsystems. | Accepted |
