...
The plugin count balloons from "everyone uses the default" to "everyone composes a recipe." A document management recipe is 10 packages; an SSG recipe is 12. Without auto-install, every recipe shift is "edit `package.json`, `npm install`, hit a missing plugin, repeat" — friction the substrate move would otherwise hide behind. **The plugin model only fully pays off if adding a plugin is as cheap as listing it in config.**

### Shape

`mikser.config.js` lists plugins. The engine scans them at `onInitialize`, identifies which are not resolvable via the existing four-step lookup chain, and installs them in one batched call before any plugin lifecycle fires.
...
- **Persistence.** Auto-install runs with `--save`, so `package.json` records the dep. Future invocations skip auto-install for the plugins already there.

- **Version pinning.** Pin to the engine's major: `mikser-io@9.x` installs `mikser-io-layouts@^9` (matching major). Prevents accidental cross-major drift. Users can override per-plugin with explicit version in the config object form (`{name, package, version}`).

- **Disabled in production.** `NODE_ENV=production` or explicit `--no-auto-install` skips it. Production servers shouldn't install mid-run; they should ship with the deps baked in. The fallback path is the existing "plugin not found" error, now with the exact `npm install` line to copy.

- **Safe-by-name.** Only auto-install packages matching the `mikser-io-*` convention (or the explicit `package:` override). Catches typos before they install random packages — `plugins: ['layoutz']` errors at the resolution step instead of silently installing `mikser-io-layoutz`.

- **Loud logging.** Every install is logged with: the resolved package name, the exact command run, the install duration, and the result. Operator sees what happened.

- **Network failure → clear fallback.** No network, no install. Falls back to the per-missing-plugin error message with the manual install command. Same shape as the schema-version recovery in 8.3.7 — the engine explains what's wrong and how to fix it.

### CLI

```
mikser                       # auto-install missing plugins (dev default)
mikser --no-auto-install     # disable; fail with "plugin not found" instead
mikser --install             # pre-warm: install missing plugins and exit (for CI prep)
mikser --package-manager=pnpm   # override lock-file detection
```

### Why default to on

The 9.0 substrate move trades "one big bundle that does everything" for "many small focused plugins that compose." Without auto-install, that trade is "easier to grok, harder to install." With auto-install, it's "easier to grok AND just as easy to install."

CI builds typically run `npm install` once at the top of the job — explicit install step covers them already; auto-install is a no-op (everything's there). Production deploys ship a built artifact with deps baked in and `NODE_ENV=production` set; auto-install opts off automatically.

The default-on choice is for the daily case: dev, prototyping, CI prep, and mid-cycle plugin additions during watch-mode development. That's the path that needs to feel frictionless.

### What this isn't

- **Not a package manager.** Mikser shells out to npm/yarn/pnpm/bun. No proprietary install logic, no opinion about lockfiles, no version solver. The engine just decides which packages need to be installed and asks the user's existing tool to do it.
- **Not silent.** Every install is logged loudly. No "magic" happens; the operator sees exactly what was installed and why.
- **Not a cross-major migration tool.** Pins to the matching major. Pre-9 plugins don't auto-upgrade through the auto-install path; they need an explicit `npm install mikser-io-X@9` from the user.
...
7. **Auto-install + watch-mode interaction.** If a user edits `mikser.config.js` mid-watch to add a new plugin, does mikser auto-install during the re-load? Working proposal: **yes** — the watch-mode re-load path goes through the same plugin scanner; missing plugins trigger the install before the new lifecycle starts. The mid-watch experience is "edit config, mikser installs + restarts, watch resumes" without operator intervention.

## Concretely, what work this is

Roughly in dependency order:

1. **mikser-io engine** — slim `src/plugins/`, add legacy stub error files at the old paths, document the `runtime.options.pipelines` surface in `documentation/api-reference.md`. Pipeline functions are no-op until a provider plugin registers them. **Auto-install of missing plugins** in the plugin loader (see "Auto-installing plugins" above) lands here too — same release, so the new plugin-rich world doesn't feel painful to set up.
