...
**Run-time MCP server.** Start mikser with `--mcp` and any MCP-compatible client — Claude Desktop, Claude Code, ChatGPT, custom agents — can drive the engine over JSON-RPC: list and read the catalog, write new content files, render layouts for preview, and watch every log line stream in as a notification. Plugins register their own tools the same way they mount Express routes. See [`documentation/mcp.md`](./documentation/mcp.md) for the tool surface and twelve end-to-end scenarios.
