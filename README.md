...
**Live MCP control plane.** The points above are about *static-time* agent friendliness — what an agent can see by reading the tree at edit time. MCP is the runtime half. Start mikser with `--mcp` and any MCP-compatible client (Claude Desktop, Claude Code, ChatGPT, custom agents) connects to the *running* engine over JSON-RPC: it lists and reads the catalog, writes new content files (writes land on disk and the next lifecycle cycle picks them up), renders layouts for preview without dirtying the catalog or the output folder (`options.save: false`), and receives every engine log line as a notification the instant it fires. Five built-in tools — `mikser_list_entities` / `_read_entity` / `_update_entity` / `_delete_entity` / `_render` — plus four `mikser://` introspection resources (`lifecycle`, `runtime`, `config`, `logs/recent`) ship out of the box; each plugin extends the surface the same way it mounts Express routes.

**Why it's in core, not a side project.** Without this, every team that wants AI-driven authoring writes a one-off MCP layer over their CMS — custom auth, custom tool shapes, a schema the agent has to relearn per project, parallel state to keep in sync. Mikser owns the layer because the engine already understands content shape: collections, layouts, the render pipeline, the lifecycle phases. The agent talks to the thing that already knows what "draft an invoice and render it as PDF" decomposes into. There's no parallel state and no schema translation — `mikser_update_entity` is the same write path your editor uses when you save a file, and the next build cycle treats AI-authored content identically to hand-authored content. That mattered enough to mark the v7.0 line and was deliberate enough to get its own architecture decision record ([ADR-0006](./documentation/decisions/0006-when-to-add-to-core.md)) — same engine, new transport, new category of capability.

**What that unlocks in practice.** *"Draft three hero-section variants and show me previews"* becomes one chat turn: three layout writes, three previews returned as inline PDF/HTML, no disk litter from rejected variants. *"Why didn't the navigation update?"* becomes one `mikser_ping` and a `mikser://logs/recent` read against the rolling 500-line buffer — the agent catches up on what scrolled past before its session connected. Multiple clients share one engine — operator + AI + an observer dashboard all see the same catalog and the same broadcasted log stream — because mikser is single-tenant by design. See [`documentation/mcp.md`](./documentation/mcp.md) for the full tool surface and twelve end-to-end scenarios.
