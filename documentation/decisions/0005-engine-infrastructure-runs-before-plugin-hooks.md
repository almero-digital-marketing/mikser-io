...
To make this work, engine.js's folder resolution (workingFolder, runtimeFolder, outputFolder) moves from `onInitialized` into `onInitialize` — the first phase. By `onInitialized`, the runtime folder exists on disk and the engine's sqlite database is open at `runtime/mikser.sqlite` with every registered schema applied (see [ADR-0009](./0009-database-engine-substrate.md)).
