...
**Static-first content with a built-in live channel — most engines pick one side, mikser does both.** Static-site generators (Hugo, Eleventy, Astro) ship fast static output but no live edit channel — every change is a rebuild. Headless CMSes (Sanity, Contentful, Strapi) ship a live API but no static essence — every page load is at least one round-trip. Mikser composes both: content publishes as static files by default, and the live channel is the layer on top, not a separate architecture. Technically: the `data` plugin ships in the engine because static publishing is the platform's essence — every catalog and entity declared under `data.*` lands as a JSON file in `out/data/`, served by mikser's built-in static handler. Frontend SDKs read from those files first; the live API is for SSE deltas and fallback, not first paint. Even the dynamic-routes scenario falls back to static files via the api plugin's `cache: true`. Pure SPA, hybrid SSG, islands, dynamic SPA — all variations on the same static-first posture, differing only in where the live channel attaches.
